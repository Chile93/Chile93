
# 👋 Hi, I'm Chinedu Emmanuel Agwunobi

🚀 **Data Engineer | Cloud Architect | Solution Architect | Data Scientist |**

I design and build scalable data pipelines, cloud-based architectures, and advanced analytics solutions. Passionate about transforming raw data into actionable insights using modern tools and platforms.

---

### 🔍 About Me
- 🌍 Based in London, UK
- 🎓 MSc in Data Science (Distinction) | Teesside University
- 🎓 BSc Mathematics – Obafemi Awolowo University 
- 💼 2+ years experience in Data Engineering & Cloud Solutions
- ☁️ Certified in AWS, Azure, and GCP
- 📊 Skilled in Python, SQL, Spark, Airflow, Databricks, and Big Data technologies

---

### 🛠️ Tech Stack
**Languages:** Python, R, SQL  
**Big Data:** Apache Spark, PySpark, Hadoop, Airflow  
**Cloud:** AWS, Azure, GCP  
**Data Warehousing:** Snowflake, Databricks, Redshift, PostgreSQL  
**Visualization:** Power BI, Tableau, QuickSight  

---

### 📌 Featured Projects
- [Real-Time Stock Market Pipeline](https://github.com/Chile93/Kafka-Stock-Market-Data-Enginnering-Project.git) – Apache Kafka, AWS Lambda, Spark Streaming  
- [YouTube Analytics Pipeline](https://github.com/Chile93/AWS-Data-Pipeline-for-YouTube-Analytics.git) – AWS Glue, Lambda, QuickSight
- [Spotify ETL Pipeline with Airflow](https://github.com/Chile93/Spotify-Airflow-Data-Pipeline.git) – Airflow, AWS EC2, Python   
- [Tokyo Olympics Analysis](https://github.com/Chile93/Olympic-Data-Analytics-Azure-End-To-End-Data-Engineering-Project.git) – Azure Databricks, PySpark, Power BI
- [Secure Application Deployment on AWS using Well-Architected Framework (Security Pillar)](https://github.com/Chile93/Secure-Application-Deployment-on-AWS-Security-Pillar-.git) - AWS, VPC, EC2, RDS, S3, ALB, WAF, IAM, CloudFront
- [Highly Available Architecture on AWS (Reliability Pillar)](https://github.com/Chile93/Highly-Available-Architecture-on-AWS-Reliability-Pillar-.git) – AWS EC2, Apache, GitHub Automation   

---

### 🏅 Certifications
- **AWS**:  
  - AWS Certified Cloud Practitioner  
  - AWS Machine Learning Foundation  
  - AWS Data Engineer  
  - AWS Solution Architect  

- **Microsoft Azure**:  
  - Azure Fundamentals (AZ-900)  
  - Azure AI Fundamentals (AI-900)  
  - Azure Data Fundamentals (DP-900)  
  - Azure Data Fabric Engineer (DP-700)  
  - Azure Administrator Associate (AZ-104)  
  - Azure Solution Architect (AZ-305)  

- **Google Cloud**:  
  - Google Cloud Associate Solution Architect  
  - Google IT Support Specialization  

- **Databricks**:  
  - Databricks Fundamentals  
  - Associate Data Engineer  

- **Other**:  
  - Udacity AI Programming with Python  
  - WordQuant Applied Data Science Lab  

---

### 📫 Connect With Me
- [LinkedIn](https://www.linkedin.com/in/chinedu-agwunobi-b33939b3/)
- 📧 chinedu.e.agwunobi@gmail.com


---

### 📊 GitHub Stats
https://github-readme-stats.vercel.app/api?username=Chile93&show_icons=true&theme=radical

https://github-readme-stats.vercel.app/api/top-langs/?username=Chile93&layout=compact&theme=radical

